...
//                    Debug.Assert(StartingVcn == 0);
        ...
        internal static extern bool OpenProcessToken([In] IntPtr ProcessToken, [In] int DesiredAccess, [In, Out] ref IntPtr TokenHandle);
        ...
        private static int AquireRights()
        {
            // Value to hold result of function
            int ReturnValue = 0;
            ...
            if (false == OpenProcessToken(ProcessHandle, TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, ref TokenHandle))
                return 1;

            // Look up LUID value of privilege
            if (false == LookupPrivilegeValue(null, SE_BACKUP_NAME, out BackupLUID))
                ReturnValue = 1;

            // Look up LUID value of privilege
            if (false == LookupPrivilegeValue(null, SE_RESTORE_NAME, out RestoreLUID))
                ReturnValue = 1;
            ...
            TokenPrivileges.Privileges.Luid = BackupLUID;
            TokenPrivileges.Privileges.Attributes = SE_PRIVILEGE_ENABLED;

            // Set privilege
            if (false == AdjustTokenPrivileges(TokenHandle, false, ref TokenPrivileges, 0, IntPtr.Zero, IntPtr.Zero))
                ReturnValue = 1;

            // Fill token data
            TokenPrivileges.PrivilegeCount = 1;
            TokenPrivileges.Privileges.Luid = RestoreLUID;
            TokenPrivileges.Privileges.Attributes = SE_PRIVILEGE_ENABLED;
        ...
        public static int ElevatePermissions()
        {
            return AquireRights();
        }
